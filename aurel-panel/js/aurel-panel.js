...
jQuery(document).ready(function(jQ) {
	...
	jQ('#aurel-panel-button-save-options').click(function() {
		...
		jQ('.aurel-panel-slider').each(function() {
			...
			slider['options'] = apo;
			if ( jQ(this).find('.aurel-panel-slider-content-toggable').hasClass('visible') ) {
				slider['visible'] = 'yes';
			} else {
				slider['visible'] = 'no';
			}
			...
			jQ(this).find('.aurel-panel-slide').each(function() {
				var s = {};
				s['image_url'] = jQ(this).find('.aurel-panel-slide-image-url').val();
				s['caption'] = jQ(this).find('.aurel-panel-slide-caption').val();
				s['caption_color'] = jQ(this).find('.aurel-panel-slide-caption-color').val();
				s['caption_background'] = jQ(this).find('.aurel-panel-slide-caption-background').val();
				s['caption_opacity'] = jQ(this).find('.aurel-panel-slide-caption-opacity').val();
				s['css'] = jQ(this).find('.aurel-panel-slide-css').val();
				aps.push(s);
			});
			slider['slides'] = aps;
			apsm.push(slider);
		...
		function populate_array_with(id) {
			...
			jQ(id + ' .aurel-panel-posts-rule').each(function() {
				...
				r['display-category-title'] = jQ('input:radio[name=ap-posts-rule-' + n + '-display-category-title]:checked').val();
				var m = new Array();
		...
		jQ('#ap_translation_manager').val(ko.toJSON(stringTable));
		
		var mobset = {};
		mobset['logo-url'] = jQ('#ap-mobile-logo-img').val();
		mobset['reducing-scale-factor'] = jQ('#ap-mobile-logo-reducing-scale').val() 
		jQ('#ap_logo_mobile_settings').val(JSON.stringify(mobset));
	...
	if (v = jQ.cookie('ap_cookie')) {
		...
		//jQ('#aurel-panel-wrap').height(6000);
		jQ('#aurel-panel-content').height(ap_size[3]);
		//jQ('#aurel-panel-content').height(6000);
	}
	...
	function init_image_upload() {
		...
		jQ('body').on('click', '.aurel-panel-upload-button', function() {
			...
			if ( jQ(this).hasClass('for-slide') ) {
				img_wrapper = jQ(this).parents('.aurel-panel-slide').find('.aurel-panel-slide-thumbnail');
			}
			return false;
		});
		window.send_to_editor = function(html) {
			//var imgurl = jQ('img',html).attr('src');
			var regex = /src="(.+?)"/;
            var rslt = html.match(regex);
            var imgurl = rslt[1];
			input_upload.val(imgurl);
			if ( img_wrapper != '' ) {
				img_wrapper.html( '<img src="' + imgurl + '" width="150" />' );
			} 
	...
	jQ('#aurel-panel-add-slider').click(function() {
		if ( /^\w+$/.test( jQ('#aurel-panel-slider-name').val() ) ) {
			if ( jQ('#aurel-panel-slider-type').val() != 'Revolution slider' ) {
	...
	jQ('.aurel-panel-slider-content-toggable').each(function() {
		if ( !jQ(this).hasClass('visible') ) {
			jQ(this).css('display', 'none');
		}
	});
	jQ('body').on('click', '.aurel-panel-slider-toggle', function() {
		var sct = jQ(this).parent().find('.aurel-panel-slider-content-toggable');
		if ( sct.hasClass('visible') ) {
			sct.removeClass('visible').slideUp();
		} else {
			sct.addClass('visible').slideDown();
		}
		return false;
	});
	
	jQ('body').on('click', '.aurel-panel-add-slide-before', function() {
		jQ(this).parent().find('.aurel-panel-slides').prepend('<div class="aurel-panel-slide">' + jQ('#aurel-panel-slide-markup').html() + '</div>');
	...
	jQ('body').on('click', '.aurel-panel-add-slide-after', function() {
		jQ(this).parent().find('.aurel-panel-slides').append('<div class="aurel-panel-slide">' + jQ('#aurel-panel-slide-markup').html() + '</div>');
		jQ(this).parent().find('.aurel-panel-slide:last').slideDown();	
		init_slides_sortable();
		init_color_picker();
		jQ(this).parent().find('.aurel-panel-slide:last .miniColors-trigger').eq(1).css('display','none');
		jQ(this).parent().find('.aurel-panel-slide:last .miniColors-trigger').eq(3).css('display','none');
	});
	...
	function posts_rule(n) {	
		...
		'<div style="display: none" class="aurel-panel-posts-rule rule-sortable">' +
			...
			'<div class="aurel-panel-rule-odd">' +
				...
				'</p>' +
				'<p>' +
					'<input id="ap-posts-rule-' + n + '-display-category-title-yes" type="radio" name="ap-posts-rule-' + n + '-display-category-title" value="yes" checked /> <label for="ap-posts-rule-' + n + '-display-category-title-yes">Yes</label>&nbsp;&nbsp;&nbsp;&nbsp;' +
					'<input id="ap-posts-rule-' + n + '-display-category-title-no" type="radio" name="ap-posts-rule-' + n + '-display-category-title" value="no" /> <label for="ap-posts-rule-' + n + '-display-category-title-no">No</label>' +
				'</p>' +
			'</div>' +
			'<div class="aurel-panel-rule-even">' +
				'<p>' +
	...
	/* 6) string translator */
	var saved_strings = JSON.parse( jQ('#ap_translation_manager').val() );
	/*
	var saved_langs = [
		{code: 'Default'},
		{code: 'en_US'},
	...
	*/
	var saved_langs = saved_strings['langs'];
	var saved_strings_value = saved_strings['strings'];
	/*
	var saved_strings_value = [
			{
				value:{
					'Default':'Learn more',
					'en_US':'Learn more',
	...
	*/
	...
	var StringTableModel = function() {
		var self = this;
		self.strings = ko.observableArray(saved_strings_value);
		self.langs = ko.observableArray(saved_langs);
		self.addString = function() {
			var new_string = { value: {} };
			for (var i=0; i<self.langs().length;  i++) {
				new_string.value[self.langs()[i].code] = 'default';
			}
			self.strings.push(new_string);
		};
		self.deleteString = function() {
			if ( confirm( 'Delete the "' + this.value['Default'] + '" string?' ) ) {
				self.strings.remove(this);
			}
		};
		self.addLang = function() {
			var lang_code = jQ('#lang-code').val();
			if ( lang_code == '' ) {
				alert('Enter a language code.');
				jQ('#lang-code').focus();
			} else {
				jQ('#lang-code').val('');
				for (var i=0; i<self.strings().length;  i++) {
					self.strings()[i].value[lang_code] = 'default';
				}
				self.langs.push({code: lang_code});
			}
		};
		self.deleteLang = function() {
			if ( confirm( 'Delete all the "' + this.code + '" strings?' ) ) {
				self.langs.remove(this);
			}
		};
	};
	
	var stringTable = new StringTableModel();
	ko.applyBindings(stringTable);
	/* end 6) string translator */
	
	/* 7) mobile logo */
	if ( jQ('input[name="ap_logo_mobile_settings_enabled"]:checked').val() == 'yes' ) {
		jQ('#ap-mobile-logo-settings').css('display','block');
	} else {
		jQ('#ap-mobile-logo-settings').css('display','none');
	}
	jQ('input[name="ap_logo_mobile_settings_enabled"]').click(function() {
		jQ('#ap-mobile-logo-settings').slideToggle();
	});
	/* end 7) mobile logo */
